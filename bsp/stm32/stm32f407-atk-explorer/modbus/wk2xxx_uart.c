...
void Wk5678UartInit(uint8_t port)
{
    ...
    gena=Wk5678ReadGlobalRegister(WK2XXX_GENA_REG);
	  ...
	  Wk5678WriteSlaveRegister(port,WK2XXX_RFTL_REG,0X40);//设置接收触点为64个字节
	  Wk5678WriteSlaveRegister(port,WK2XXX_TFTL_REG,0X10);//设置发送触点为16个字节
...
#if 1
	Wk5678WriteSlaveFifo(port,sendbuf,len);//通过fifo方式发送数据
	return 0;
#else
	uint16_t num=len;
...
{   
	  Wk5678WriteSlaveRegister(port,WK2XXX_FWCR_REG,0x30);//
		Wk5678WriteSlaveRegister(port,WK2XXX_SPAGE_REG,1);//切换到page1
		Wk5678WriteSlaveRegister(port,WK2XXX_FWTH_REG,0XF0);//停止接收触点
		Wk5678WriteSlaveRegister(port,WK2XXX_FWTL_REG,0X20);//继续接收触点
		Wk5678WriteSlaveRegister(port,WK2XXX_SPAGE_REG,0);//切换到page0 
}
...
{ 
	...
	do{
			...
			if(gifr&GIFR_UT1INT_BIT){ /*数据处理*/
					/*数据接收*/
					 rxlen=Wk5678UartRxChars(1,rxbuf);//一次接收的数据不会超过256Byte
					/*数据发送*/
					//把接收的数据发送出去
					 if(rxlen>0){
			...
			if(gifr&GIFR_UT2INT_BIT){
				/*数据接收*/
						rxlen=Wk5678UartRxChars(2,rxbuf);//一次接收的数据不会超过256Byte
						/*数据发送*/
						//把接收的数据发送出去
						if(rxlen>0){
			...
			if(gifr&GIFR_UT3INT_BIT){
				    /*数据接收*/
						rxlen=Wk5678UartRxChars(3,rxbuf);//一次接收的数据不会超过256Byte
						/*数据发送*/
						//把接收的数据发送出去
				    if(rxlen>0){
			...
			if(gifr&GIFR_UT4INT_BIT){
				    /*数据接收*/
						rxlen=Wk5678UartRxChars(4,rxbuf);//一次接收的数据不会超过256Byte
						/*数据发送*/
						//把接收的数据发送出去
				    if(rxlen>0){
