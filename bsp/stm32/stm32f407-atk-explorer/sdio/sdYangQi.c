    ...
    int ret;
		...
		if(ret!=FR_OK){
		    ret=f_open(&fnew,txtName, FA_CREATE_NEW |FA_WRITE);//suqi
			  if(ret==FR_OK){
					ret=f_write(&fnew,o2Text,strlen(o2Text),&fnum);//写入name列表
...
void o2DelEarlyTxt()
{
		...
		{		
			  if(sheet.o2[i].workFlag==RT_TRUE){
						ReadAndDelEarlyDataTxt((char *)modbusName[O2],sheet.o2[i].ID);
				}
		}
}