#include "board.h"
static const char sign[]="[SDShuiWei]";
//在 creatFolder/ID号创建的文件夹下边创建带有时间的txt文件
//创建txt文件 以utc时间戳为基准/3600*24 得到每天的时间戳作为txt文件的名称

static const char shuiWeiText[]="depth time(ms)\r\n";


static uint32_t writetime=0;
void shuiWeiSaveSD(char *id,char *data)
{
		if(gbSDExit==false){
				return;
		}
    ...
    int ret;
		...
		strcpy(txtName,modbusName[WATERDEPTH]);
		strcat(txtName,"/");
		strcat(txtName,id);
		strcat(txtName,"/");
		...
		if(ret!=FR_OK){
		    ret=f_open(&fnew,txtName, FA_CREATE_NEW |FA_WRITE);//suqi
			  if(ret==FR_OK){
					ret=f_write(&fnew,shuiWeiText,strlen(shuiWeiText),&fnum);//写入name列表
...
//
void shuiWeiDelEarlyTxt()
{
		for (int i = 0; i < WATERDEPTH_485_NUM; i++)
		{		
			  if(sheet.waterDepth[i].workFlag==RT_TRUE){
						ReadAndDelEarlyDataTxt((char *)modbusName[WATERDEPTH],sheet.waterDepth[i].ID);
				}
		}
}