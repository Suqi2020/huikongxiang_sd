#include "board.h"
static const char sign[]="[SDJingGai]";
//在 creatFolder/ID号创建的文件夹下边创建带有时间的txt文件
//创建txt文件 以utc时间戳为基准/3600*24 得到每天的时间戳作为txt文件的名称

static const char jingGaiText[]="incline switch2(2盖) vibration switch1(1盖) time(ms)\r\n";


static uint32_t writetime=0;
void jingGaiSaveSD(char *id,char *data)
{
		if(gbSDExit==false){
				return;
		}
    ...
    int ret;
		...
		strcpy(txtName,modbusName[COVER]);
		strcat(txtName,"/");
		strcat(txtName,id);
		strcat(txtName,"/");
		...
		if(ret!=FR_OK){
		    ret=f_open(&fnew,txtName, FA_CREATE_NEW |FA_WRITE);//suqi
			  if(ret==FR_OK){
					ret=f_write(&fnew,jingGaiText,strlen(jingGaiText),&fnum);//写入name列表
...
//
void jingGaiDelEarlyTxt()
{
		for (int i = 0; i < COVER_485_NUM; i++)
		{		
			  if(sheet.cover[i].workFlag==RT_TRUE){
						ReadAndDelEarlyDataTxt((char *)modbusName[COVER],sheet.cover[i].ID);
				}
		}
}