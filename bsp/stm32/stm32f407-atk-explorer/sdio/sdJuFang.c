#include "board.h"
static const char sign[]="[SDJuFang]";
...
void juFangTxtSaveSD(char *id,char *data)
{
		if(gbSDExit==false){
				return;
		}
    ...
    int ret;
		...
		strcpy(txtName,modbusName[PARTDISCHAG]);
		strcat(txtName,"/");
		strcat(txtName,id);
		strcat(txtName,"/");
		...
		if(ret!=FR_OK){
		    ret=f_open(&fnew,txtName, FA_CREATE_NEW |FA_WRITE);//suqi
			  res_sd=f_write(&fnew,juFangText,strlen(juFangText),&fnum);//写入name列表
...
//
void juFangDelEarlyTxt()
{
		for (int i = 0; i < PARTDISCHAG_485_NUM; i++)
		{		
			  if(sheet.partDischag[i].workFlag==RT_TRUE){
						ReadAndDelEarlyDataTxt((char *)modbusName[PARTDISCHAG],sheet.partDischag[i].ID);
				}
		}
}