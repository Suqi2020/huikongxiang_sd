    ...
    int ret;
		...
		if(ret!=FR_OK){
		    ret=f_open(&fnew,txtName, FA_CREATE_NEW |FA_WRITE);//suqi
			  if(ret==FR_OK){
					ret=f_write(&fnew,coText,strlen(coText),&fnum);//写入name列表
...
void coDelEarlyTxt()
{
		...
		{		
			  if(sheet.co[i].workFlag==RT_TRUE){
						ReadAndDelEarlyDataTxt((char *)modbusName[CO],sheet.co[i].ID);
				}
		}
}