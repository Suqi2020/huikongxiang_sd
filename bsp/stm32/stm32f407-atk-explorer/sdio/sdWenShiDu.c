#include "board.h"
static const char sign[]="[SDWenShiDu]";
//在 creatFolder/ID号创建的文件夹下边创建带有时间的txt文件
//创建txt文件 以utc时间戳为基准/3600*24 得到每天的时间戳作为txt文件的名称

static const char tempHumText[]="temperature humidity time(ms)\r\n";


static uint32_t writetime=0;
void wenShiDuSaveSD(char *id,char *data)
{
		if(gbSDExit==false){
				return;
		}
    ...
    int ret;
		...
		strcpy(txtName,modbusName[TEMPHUM]);
		strcat(txtName,"/");
		strcat(txtName,id);
		strcat(txtName,"/");
		...
		if(ret!=FR_OK){
		    ret=f_open(&fnew,txtName, FA_CREATE_NEW |FA_WRITE);//suqi
			  if(ret==FR_OK){
					ret=f_write(&fnew,tempHumText,strlen(tempHumText),&fnum);//写入name列表
...
//
void wenShiDuDelEarlyTxt()
{
		for (int i = 0; i < TEMPHUM_485_NUM; i++)
		{		
			  if(sheet.tempHum[i].workFlag==RT_TRUE){
						ReadAndDelEarlyDataTxt((char *)modbusName[TEMPHUM],sheet.tempHum[i].ID);
				}
		}
}
