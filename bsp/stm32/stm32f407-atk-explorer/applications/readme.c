...
//V4.07    增加工装测试代码                      								20230814
//V4.08    完善工装uart测试代码支持rs485(0-8)9个串口测试        20230817
//V4.09    修复上电默认0xff时候为MQTT协议的错误                 20230818
//V4.10    增加SD线程的看门狗事件标志组													20230821
//V4.11    增加天井洼定制传感器数据打包的代码    
//         增加LCD显示modbus传感器数据时候index紊乱的修复       20230829
//V4.12    完善天井洼定制传感器的其他显示代码                   20230830
//V4.13    修改传感器数据显示界面翻页选择为图标选择             
//V4.14    修改传感器汉字显示界面拼音为中文显示                 20230831
//V4.15    调整部分传感器温度和数值显示顺序                     20230901
//V4.16    增加mosbus传感器的状态显示（在线 离线 无）功能       20230906
//V4.17    根据天井洼定制再次增加若干功能                       20230912
//V4.18    增加未配置的传感器显示”离线“更改为”无“功能           20231020
//V4.19    增加读取时候冗余数据滤波功能，
//         修改串口读取数据多个读取为区分为单个读取							20231031
//V4.20    修改3秒校准时间为30秒，
...
//                                                              20231215
//4.23    1、去掉读取寄存器数据成功时候break
//				2、中断来临读取寄存器值count为0 异常时候复位前重新初始化
//           主串口和关IO中断，初始化wk完成后开IO中断
//				3、初始化完成后读取WK2XXX_GENA_REG寄存器核对 如果读出不对 
//            反复延时初始化 最多5次                            20231228
//4.24   1、修复创建H2S O2 co 水位 防沉降时候创建log异常
//       2、去掉重新初始化串口时候关外部9-15中断 去掉中断函数中二次清串口中断
//           增加串口初始化读串口WK2XXX_GENA_REG寄存器时候信号量接收 20240103
//V4.25  更正显示屏和子站打包数据中的三轴测振仪的负数显示 
//       当采集值大于0x8000时候显示负数     
//       加入sprintf_threeAxis函数和sprintf_threeAxismg函数    20240108
