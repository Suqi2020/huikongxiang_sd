...
void  dispHuanliuData()
{
	  ...
	  uint8_t buf[MODBID_LEN+2]={0};
		...
		if(dispHuanliuTotlNum==0){//没有工作的环流
			  ...
			  LCDDispModbusState(-1,DISP_DATA_HUANLIU_STATE_ADDR);
		}
		else{
...
void  dispHuanliuData_tjw()
{
	  uint8_t buf[MODBID_LEN+2]={0};
		...
		if(dispHuanliuTotlNum==0){//没有工作的环流
				...
				LCDDispModbusState(-1,DISP_DATA_HUANLIU1_STATE_TJW_ADDR);
				LCDDispModbusState(-1,DISP_DATA_HUANLIU2_STATE_TJW_ADDR);
